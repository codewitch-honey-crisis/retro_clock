...
static const open_font& clock_font = DSEG14Classic_Regular;
#else
#define DSEG7CLASSIC_REGULAR_IMPLEMENTATION
#include <assets/DSEG7Classic_Regular.hpp>
static const open_font& clock_font = DSEG7Classic_Regular;
#endif
#if !defined(E_PAPER)
...
void calculate_positioning() {
    ...
    float scl = clock_font.scale(lcd.dimensions().height - 2);
    ssize16 dig_size = clock_font.measure_text(ssize16::max(), spoint16::zero(), "0", scl);
...
#endif
    if(am_pm) {
      am_pm_size = clock_font.measure_text(ssize16::max(), spoint16::zero(), ".", scl);
      w+=am_pm_size.width;
    }
    ...
    #endif
    oti=open_text_info(str,clock_font,clock_font.scale(lh));
    text_bounds = (rect16)clock_font.measure_text(
      ssize16::max(),
      oti.offset,
      oti.text,
      oti.scale,
